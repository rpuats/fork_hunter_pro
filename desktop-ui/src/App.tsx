...
function App() {
  ...
  const { connected, scannerStatus, surebets, metrics, bookmakers, corridors, expressForks, valueBets, generosityIndices, executionOverview, executionLedger, executionState, executionOperatorQueue, parserCoverage, parserHealth, accounts, accountsSummary, bankrollState, bankrollRecommendations, freebetSummary, bookmakerStatusCatalog } = useScanner()
  ...
  const renderPage = () => {
    switch(activeTab) {
      ...
      case 'operator':
        return <OperatorPage executionOverview={executionOverview} executionLedger={executionLedger} executionState={executionState} executionOperatorQueue={executionOperatorQueue} parserCoverage={parserCoverage} parserHealth={parserHealth} bookmakers={bookmakers} bookmakerStatusCatalog={bookmakerStatusCatalog} accountStates={accounts} freebetSummary={freebetSummary} onOpenAccount={openAccountsFocus} />
      case 'accounts':
