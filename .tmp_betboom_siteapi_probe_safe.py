...
for path in paths:
    for body in bodies:
        try:
            r = requests.post(base + path, headers=headers, data=json.dumps(body), timeout=20)
            out.append({'path': path, 'body': body, 'status': r.status_code, 'text': r.text[:220]})
            if r.status_code == 200 and '"code":404' not in r.text:
                break
        except Exception as e:
            out.append({'path': path, 'body': body, 'error': str(e)[:220]})
            break
