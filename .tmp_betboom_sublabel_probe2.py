...
TARGETS=[('Футбол',['Россия','Англия','Испания','Италия','Франция','Бразилия']),('Бейсбол',['Япония','США','KBO'])]
async def main():
    ...
    async with async_playwright() as pw:
        ...
        for cat,subs in TARGETS:
            ...
            await p._click_visible_text(page, '1н')
            await asyncio.sleep(4)
            await p._click_visible_text(page, cat)
            await asyncio.sleep(4)
            cat_out=[]
            for sub in subs:
                clicked=await p._click_visible_text(page, sub, prefix=True)
                if not clicked:
