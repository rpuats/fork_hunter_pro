# scrapers/betboom_scraper.py - BetBoom scraper для вилок
import asyncio
import re
import logging
...
class BetBoomScraper(BaseScraper):
    def __init__(self):
        super().__init__()
        self.name = "BetBoom"
        
        # ОСЛАБЛЕННЫЙ фильтр для BetBoom (для вилок)
    ...
    async def get_events(self) -> List[Dict]:
        ...
        try:
            async with async_playwright() as p:
                ...
                await page.goto("https://betboom.ru/live/football", wait_until="domcontentloaded", timeout=90000)
            ...
            logger.error(f"[BetBoom] Error: {e}")

        logger.info(f"[BetBoom] Получено {len(events)} событий")
        return events
