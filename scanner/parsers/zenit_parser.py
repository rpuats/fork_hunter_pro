...
class ZenitParser(BaseParser):
    name = "Zenit"
    slug = "zenit"
    base_url = "https://zenitbet.com"
    
    async def get_events(self) -> List[Dict]:
        events = []
        
        urls = [
            "https://zenitbet.com/api/live",
            "https://zenitbet.com/live",
        ]
            ...
            try:
                ...
                if data:
                    ...
                    if parsed:
                        ...
                        logger.info(f"Zenit: got {len(parsed)} events from {url}")
                        break
            except Exception as e:
                logger.debug(f"Zenit: failed to fetch {url}: {e}")
                continue
        
        logger.debug(f"Zenit: total {len(events)} events collected")
        return events[:50]
    ...
    def _parse_response(self, data: dict) -> List[Dict]:
        ...
        for item in items if isinstance(items, list) else []:
            try:
                ...
                logger.debug(f"Zenit: failed to normalize event: {e}")
                continue
    ...
    def _normalize_event(self, raw: Dict) -> Optional[Dict]:
        try:
            ...
            return {
                'id': f"zenit_{raw.get('id', hash(home + away))}",
                'bookmaker': 'zenit',
                'sport': 'football',
                'home_team': home,
                'away_team': away,
            ...
            logger.debug(f"Zenit: error normalizing event: {e}")
            return None
