# scanner/parsers/sportbet_playwright.py
"""Sportbet Parser - Text extraction"""
import asyncio, time
...
class SportbetPlaywrightParser:
    name = "Sportbet (Playwright)"
    slug = "sportbet"
    urls = ["https://sportbet.ru/live", "https://sportbet.ru/line"]
    ...
    async def get_events(self) -> List[Dict]:
        ...
        for url in self.urls:
            try:
                ...
                logger.warning(f"Sportbet failed for {url}: {e}")
        return all_events
    ...
    async def _fetch_url(self, url: str) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Sportbet ({url}): {len(events)} events")
        except Exception as e:
            logger.warning(f"Sportbet error: {e}")
            events = []
        finally:
            await browser.close()
        return events
    ...
    async def _extract_events(self, page, url: str) -> List[Dict]:
        try:
            ...
            logger.warning(f"Sportbet extraction error: {e}")
            return []
