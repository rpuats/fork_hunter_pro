# scanner/parsers/onexstavka_parser.py
import logging
...
class OnexStavkaParser(BaseParser):
    name = "1xStavka"
    slug = "1xstavka"
            ...
            try:
                ...
                if data:
                    ...
                    if parsed:
                        ...
                        logger.info(f"1xStavka: got {len(parsed)} events from {url}")
                        break
            except Exception as e:
                logger.debug(f"1xStavka: failed to fetch {url}: {e}")
                continue
        
        logger.debug(f"1xStavka: total {len(events)} events collected")
        return events[:50]
    
    def _parse_response(self, data: dict) -> List[Dict]:
        events = []
        
        items = data.get('Value', []) or data.get('events', []) or data.get('data', [])
        ...
        for item in items if isinstance(items, list) else []:
            try:
                ...
                logger.debug(f"1xStavka: failed to normalize event: {e}")
                continue
    ...
    def _normalize_event(self, raw: Dict) -> Optional[Dict]:
        try:
            ...
            return {
                'id': f"1xstavka_{raw.get('I', hash(home + away))}",
                'bookmaker': '1xstavka',
                'sport': 'football',
                'home_team': home,
                'away_team': away,
            ...
            logger.debug(f"1xStavka: error normalizing event: {e}")
            return None
