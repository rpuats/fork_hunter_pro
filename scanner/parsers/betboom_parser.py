# scanner/parsers/betboom_parser.py
"""
BetBoom parser - uses BetBoom's API endpoints with multiple fallback URLs.
BetBoom provides live and pre-match events via REST API.
"""
import time
import logging
...
class BetBoomParser(BaseParser):
    ...
    LIVE_API_URLS = [
        "https://betboom.ru/api/live/v3/events",
        "https://api.betboom.ru/live/events",
        "https://betboom.ru/api/live/football",
        "https://betboom.ru/live/api/getGames",
        "https://api.betboom.ru/v2/live/sports",
    ]

    PREMATCH_API_URLS = [
        "https://betboom.ru/api/prematch/v3/events",
        "https://api.betboom.ru/prematch/events",
        "https://betboom.ru/api/prematch/football",
    ]
    ...
    async def get_events(self) -> List[Dict]:
        ...
        for is_live in [True, False]:
            ...
            headers = self._get_betboom_headers()
                ...
                try:
                    ...
                    if data:
                        ...
                        if parsed:
                            ...
                            logger.info(f"BetBoom: got {len(parsed)} events from {url}")
                            break
                except Exception as e:
                    logger.debug(f"BetBoom: failed to fetch {url}: {e}")
                    continue

        return events[:50]

    def _get_betboom_headers(self) -> Dict[str, str]:
    ...
    def _parse_response(self, data: Any, is_live: bool = True) -> List[Dict]:
        ...
        if isinstance(data, dict):
            ...
            logger.warning(f"BetBoom: unexpected data type: {type(data)}")
            return []
        ...
        for item in items if isinstance(items, list) else []:
            try:
                ...
                logger.debug(f"BetBoom: failed to normalize event: {e}")
                continue
    ...
    def _normalize_event(self, raw: Dict, is_live: bool = True) -> Optional[Dict]:
        try:
            ...
            home_odds = self._extract_odds(coeffs, ['coefficient1', 'k1', 'w1', 'win1', '1', 'home'])
            draw_odds = self._extract_odds(coeffs, ['coefficientX', 'kx', 'draw', 'x', 'X', 'tie'])
            away_odds = self._extract_odds(coeffs, ['coefficient2', 'k2', 'w2', 'win2', '2', 'away'])
            ...
            event_id = (
                ...
                or raw.get('gameId')
                or raw.get('matchId')
                or f"betboom_{hash(home + away)}"
            )

            return {
                'id': f"betboom_{event_id}",
                'bookmaker': 'betboom',
                'sport': sport,
                'home_team': home,
                'away_team': away,
            ...
            logger.debug(f"BetBoom: error normalizing event: {e}")
            return None
