# scanner/parsers/melbet_playwright.py
"""
Melbet Playwright Parser
"""
import asyncio
import time
...
class MelbetPlaywrightParser:
    name = "Melbet (Playwright)"
    slug = "melbet"
    urls = [
        ...
        for url in self.urls:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.warning(f"Melbet attempt {attempt + 1}/{max_retries} failed for {url}: {e}")
    ...
    async def _fetch_url(self, url: str) -> List[Dict]:
        ...
        logger.info(f"Melbet ({url}): Extracted {len(events)} events")
        ...
        raw_events = await page.evaluate("""
            () => {
                ...
                const selectors = [
                    ...
                    '.sport-event', '.event-line', '.ssr-gallery'
                ];
    ...
    def _normalize_events(self, raw_events: list, url: str) -> List[Dict]:
        ...
        for i, e in enumerate(raw_events):
            ...
            event = {
                'id': f"melbet_{i}_{hash(key) % 1000000}",
                'bookmaker': 'melbet',
                'sport': 'football',
                'home_team': home,
                'away_team': away,
...
async def test():
    ...
    parser = MelbetPlaywrightParser()
    events = await parser.get_events()
    print(f'Melbet: {len(events)} events')
