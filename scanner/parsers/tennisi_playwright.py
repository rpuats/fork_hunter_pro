# scanner/parsers/tennisi_playwright.py
"""
Tennisi Playwright Parser
"""
import asyncio
import time
...
class TennisiPlaywrightParser:
    name = "Tennisi (Playwright)"
    slug = "tennisi"
    urls = [
        "https://tennisi.bet/live",
        "https://tennisi.bet/line",
    ]
        ...
        for url in self.urls:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.warning(f"Tennisi attempt {attempt + 1}/{max_retries} failed for {url}: {e}")
    ...
    async def _fetch_url(self, url: str) -> List[Dict]:
        ...
        logger.info(f"Tennisi ({url}): Extracted {len(events)} events")
    ...
    def _normalize_events(self, raw_events: list, url: str) -> List[Dict]:
        ...
        for i, e in enumerate(raw_events):
            ...
            event = {
                'id': f"tennisi_{i}_{hash(key) % 1000000}",
                'bookmaker': 'tennisi',
                'sport': 'football',
                'home_team': home,
                'away_team': away,
...
async def test():
    ...
    parser = TennisiPlaywrightParser()
    events = await parser.get_events()
    print(f'Tennisi: {len(events)} events')
