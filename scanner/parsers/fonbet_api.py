# scanner/parsers/fonbet_api.py
"""
Fonbet parser - uses official REST API (same platform as Pari/Bettery).
Endpoints:
  Live: /ma/events/list?lang=ru&scopeMarket=1600
  Prematch: /ma/events/listBase?lang=ru&scopeMarket=1600
...
class FonbetParser(BaseParser):
    ...
    LIVE_API = "https://line-lb61-w.bk6bba-resources.com/ma/events/list?lang=ru&scopeMarket=1600"
    PREMATCH_API = "https://line-lb61-w.bk6bba-resources.com/ma/events/listBase?lang=ru&scopeMarket=1600"
        ...
        for api_url, is_live in [(self.LIVE_API, True), (self.PREMATCH_API, False)]:
            try:
                ...
                if data:
                    ...
                    logger.info(f"Fonbet: got {len(parsed)} events from {'live' if is_live else 'prematch'}")
            except Exception as e:
                logger.debug(f"Fonbet: failed to fetch {api_url}: {e}")
        
        logger.info(f"Fonbet: total {len(events)} events")
        return events
    ...
    def _fetch_api_sync(self, url: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.debug(f"Fonbet API error: {e}")
        return None
