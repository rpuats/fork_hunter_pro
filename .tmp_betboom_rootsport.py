...
async def main():
    async with async_playwright() as pw:
        ...
        for url in URLS:
            requests=[]
            page.on('request', lambda req: requests.append({'url': req.url, 'method': req.method, 'resource': req.resource_type}) if ('sporthub' in req.url or 'siteapi' in req.url or '/api/games/' in req.url or 'widget.js' in req.url or req.url.startswith('wss://')) else None)
            ...
            out.append({'url': url, 'data': data, 'requests': requests[:60]})
        await browser.close()
