...
static TEAM_ALIASES: Lazy<HashMap<&str, Vec<&str>>> = Lazy::new(|| {
    ...
    map.insert(
        "Real Madrid",
        vec!["Реал Мадрид", "Реал", "Real Madrid CF", "Real"],
    );
    map.insert(
        "Barcelona",
        vec!["Барселона", "Барса", "FC Barcelona", "Barça"],
    );
    map.insert(
        "Atletico Madrid",
        vec!["Атлетико", "Атлетико Мадрид", "Atlético"],
    );
    // Английские
    map.insert(
        "Manchester United",
        vec!["Манчестер Юнайтед", "Ман Юнайтед", "Man Utd", "MUFC"],
    );
    map.insert(
        "Manchester City",
    ...
    map.insert(
        "Bayern Munich",
        vec!["Бавария", "Bayern", "FC Bayern", "Бавария Мюнхен"],
    );
    map.insert(
        "Borussia Dortmund",
        vec!["Боруссия Дортмунд", "BVB", "Боруссия Д"],
    );
    map.insert("RB Leipzig", vec!["РБ Лейпциг", "Leipzig", "Лейпциг"]);
    // Французские
    map.insert(
        "PSG",
        vec!["ПСЖ", "Paris Saint-Germain", "Пари Сен-Жермен", "Париж"],
    );
    map.insert(
        "Olympique Marseille",
        vec!["Олимпик Марсель", "Марсель", "OM"],
    );
    // Итальянские
    ...
    map.insert("Rostov", vec!["Ростов", "FC Rostov", "FK Rostov"]);
    map.insert(
        "Alaves",
        vec!["Алавес", "Deportivo Alaves", "Alavés", "Alaves"],
    );
    map.insert(
        "Juventud Las Piedras",
        vec!["Ювентуд", "Juventud", "CA Juventud"],
    );
    map.insert("Khozyaeva", vec!["Хозяева", "Home", "Хозяева"]);
    // Российские
    map.insert(
        "CSKA Moscow",
        vec!["ЦСКА", "ЦСКА Москва", "PFC CSKA", "ЦСКА М"],
    );
    map.insert(
        "Spartak Moscow",
        vec!["Спартак", "Спартак Москва", "FC Spartak"],
    );
    map.insert(
        "Zenit",
        vec!["Зенит", "Зенит СПб", "FC Zenit", "Зенит Санкт-Петербург"],
    );
    map.insert(
        "Lokomotiv Moscow",
        vec!["Локомотив", "Локо Москва", "FC Lokomotiv", "Локомотив М"],
    );
    map.insert(
        "Dynamo Moscow",
        vec!["Динамо Москва", "Динамо М", "FC Dynamo"],
    );
...
impl Normalizer {
    ...
    pub fn normalize_league(&self, league: &str) -> String {
        ...
        match lower.as_str() {
            ...
            | "английская премьер лига" => "Premier League".into(),
            ...
            // German Bundesliga
            "бундеслига" | "bundesliga" | "германия" => "Bundesliga".into(),

            // Italian Serie A
            "серия а" | "serie a" | "италия" => "Serie A".into(),

            // French Ligue 1
            "лига 1" | "ligue 1" | "франция" => "Ligue 1".into(),
            ...
            // Russian Cup
            "кубок россии" | "russian cup" => "Russian Cup".into(),
