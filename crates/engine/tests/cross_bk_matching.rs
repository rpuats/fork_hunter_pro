...
fn make_odd(id: &str, event_id: &str, bookmaker: &str, market: &str, selection: &str, odds: f64, line: Option<f64>) -> Odd {
    Odd {
        id: id.to_string(),
...
fn test_01_fingerprint_basic() {
    ...
    // Pari event
    let pari_event = make_event("pari_123", "pari", 
        "Манчестер Юнайтед", "Ливерпуль", "АПЛ");
    
    // Fonbet event — SAME MATCH but different naming
    let fonbet_event = make_event("fonbet_456", "fonbet",
        "Манчестер Юнайтед ", "Ливерпуль", "Английская Премьер-Лига");
    
    let fp_pari = event_fingerprint(&pari_event);
    let fp_fonbet = event_fingerprint(&fonbet_event);
    
    println!("  Pari:    {} vs {} ({})", pari_event.home_team, pari_event.away_team, pari_event.league);
    println!("    → FP: {}", fp_pari);
    println!("  Fonbet:  {} vs {} ({})", fonbet_event.home_team, fonbet_event.away_team, fonbet_event.league);
    println!("    → FP: {}", fp_fonbet);
    println!("  Match:  {}", if fp_pari == fp_fonbet { "✅ YES" } else { "❌ NO" });
...
fn test_02_different_leagues_no_match() {
    ...
    let event_rpl = make_event("rpl_1", "pari", 
        "Зенит", "Спартак", "РПЛ");
    
    let event_epl = make_event("epl_1", "fonbet",
        "Зенит", "Спартак", "АПЛ");  // Same teams, different league
    ...
    println!("  Match:  {}", if fp_rpl == fp_epl { "❌ YES (bad!)" } else { "✅ NO (good!)" });
...
fn test_03_various_name_formats() {
    ...
    let fps: Vec<_> = names.iter().enumerate().map(|(i, (home, away, league))| {
...
fn test_04_cross_bk_surebet_1x2() {
    ...
    if !surebets.is_empty() {
        ...
        println!("  Legs:");
        for leg in &sb.legs {
            println!("    - {} {}@{:.2} stake={:.2}", leg.bookmaker, leg.selection, leg.odds, leg.stake);
        }
    } else {
...
fn test_06_normalizer_team_matching() {
    ...
    for (input, expected_contains) in &teams {
        ...
        println!("  '{}' → '{}' (contains '{}': {})", 
            input, normalized, expected_contains,
            normalized.contains(expected_contains) || normalized.to_lowercase().contains(&expected_contains.to_lowercase()));
    }
}
...
fn test_07_league_normalization() {
    ...
    for (input, expected) in &leagues {
        ...
        println!("  '{}' → '{}' (expected '{}': {})", 
            input, normalized, expected, if pass { "✅" } else { "❌" });
        assert_eq!(normalized, *expected, "League '{}' should normalize to '{}'", input, expected);
    }
...
fn test_08_full_pipeline_simulation() {
    ...
    let events = vec![
        ...
        make_event("fonbet_200", "fonbet", "Real Madrid", "Barcelona", "Испания"),
        make_event("marathon_300", "marathon", "Реал", "Барселона ", "Primera Division"),
    ];
    ...
    for (fp, match_events) in &matches {
        ...
        let bks: std::collections::HashSet<_> = odds.iter().map(|o| o.bookmaker_slug.as_str()).collect();
        println!("    FP: {} ({} events, {} odds from {:?})", 
            fp, match_events.len(), odds.len(), bks);
        
        // Теперь ищем вилки
        let norm = Normalizer::new();
        let norm_events: Vec<Event> = match_events.iter().map(|e| norm.normalize_event((**e).clone())).collect();
        
        let calc = SurebetCalculator::new(0.5, 30.0, 1000.0, 10000, 0.01);
        let surebets = calc.find_surebets(&norm_events, &odds.iter().map(|o| (**o).clone()).collect::<Vec<_>>());
...
fn test_09_detect_real_world_naming_differences() {
    ...
    let test_cases = vec![
        ...
        (
            ...
            make_event("m2", "marathon", "ЦСКА", "Локомотив", "Российская Премьер-Лига"),
            true,
        ),
    ...
    for (i, (ev1, ev2, should_match)) in test_cases.iter().enumerate() {
        ...
        println!("  Case #{}:", i+1);
        println!("    A: {} vs {} ({})", ev1.home_team, ev1.away_team, ev1.league);
        println!("       → {}", fp1);
        println!("    B: {} vs {} ({})", ev2.home_team, ev2.away_team, ev2.league);
        println!("       → {}", fp2);
        println!("    Expected: {}, Got: {} {}", 
            if *should_match { "MATCH" } else { "NO MATCH" },
            if matches { "MATCH" } else { "NO MATCH" },
            if matches == *should_match { "✅" } else { "❌" });
