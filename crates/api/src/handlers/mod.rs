...
pub async fn health_check(State(state): State<Arc<AppState>>) -> Json<ApiResponse<serde_json::Value>> {
    let coverage = live_parsers_coverage(&state);
    Json(ApiResponse::ok(build_health_payload(&coverage)))
}

pub async fn get_metrics(State(state): State<Arc<AppState>>) -> Json<ApiResponse<ScannerMetrics>> {
...
pub async fn get_bankroll(State(state): State<Arc<AppState>>) -> Json<ApiResponse<BankrollState>> {
    let registry = execution_registry(&state);
...
fn parse_account_session_material(
    ...
    let bk_id = request.bookmaker.clone().unwrap_or_default();
    
    let material = BookmakerSessionMaterial {
        session: auto_betting::auth::AuthSession {
            bookmaker_id: bk_id.clone(),
            cookies: auto_betting::auth::SessionCookies::default(),
            created_at: Utc::now(),
            expires_at: None,
        },
        credentials: auto_betting::auth::BookmakerCredentials {
            bookmaker_id: bk_id,
            login: String::new(),
            password: String::new(),
            phone_prefix: None,
            two_fa_secret: None,
            status: auto_betting::auth::AuthStatus::NotAuthenticated,
            cookies: None,
            balance: None,
            last_auth: None,
            display_config: None,
            ...
            for agent_result in bulk_output.results {
                ...
                let result = if authenticated {
                    ...
                    let session_material = Some(BookmakerSessionMaterial {
                        session: auto_betting::auth::AuthSession {
                            bookmaker_id: agent_result.bookmaker.clone(),
                            cookies: auto_betting::auth::SessionCookies::default(),
                            created_at: Utc::now(),
                            expires_at: None,
                        },
                        credentials: auto_betting::auth::BookmakerCredentials {
                            bookmaker_id: agent_result.bookmaker.clone(),
                            login: agent_result.raw_login.clone(),
                            password: String::new(),
                            phone_prefix: None,
                            two_fa_secret: None,
                            status: auto_betting::auth::AuthStatus::Authenticated,
                            cookies: None,
                            balance: None,
...
pub async fn get_capabilities(State(state): State<Arc<AppState>>) -> Json<ApiResponse<ApiSurfacePlan>> {
