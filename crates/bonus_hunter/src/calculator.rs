...
impl BonusCalculator {
    ...
    pub fn calculate_ev(
        ...
        let real_value = Self::calculate_real_value(bonus_amount, wager_requirement, min_odds, bookmaker_margin);
    ...
    pub fn assess_difficulty(
        ...
        let bets_per_day = if expiry_days > 0 { bets_needed / expiry_days } else { bets_needed };

        let odds_penalty = if min_odds > 2.0 { 2 } else { 0 };
        let time_penalty = if expiry_days < 7 { 2 } else if expiry_days < 14 { 1 } else { 0 };
        let volume_penalty = if bets_per_day > 20 { 3 } else if bets_per_day > 10 { 2 } else if bets_per_day > 5 { 1 } else { 0 };
...
mod tests {
    ...
    fn test_difficulty_easy() {
        ...
        assert!(matches!(diff, BonusDifficulty::Easy | BonusDifficulty::Medium));
    }

    #[test]
    fn test_difficulty_hard() {
        let diff = BonusCalculator::assess_difficulty(10.0, 3.0, 3, 1000.0, 5000.0);
        assert!(matches!(diff, BonusDifficulty::Hard | BonusDifficulty::VeryHard));
    }
}
