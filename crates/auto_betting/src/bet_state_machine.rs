...
pub enum BetPlacementState {
    ...
    /// Заявка подготовлена (расчёт и preflight)
    Prepared,

    /// Заявка валидирована и готова к исполнению
    Validated,
...
impl BetPlacementStateMachine {
    ...
    pub fn new(command_id: Uuid) -> Self {
        ...
        let mut machine = Self {
            command_id,
            state: BetPlacementState::Pending,
            events: Vec::new(),
            created_at: now,
            last_updated: now,
            error_message: None,
        ...
        machine.add_event(BetPlacementEvent::Pending { timestamp: now });
    ...
    pub fn mark_prepared(&mut self) -> Result<(), String> {
        if self.state != BetPlacementState::Pending {
            return Err("Invalid state for prepared".into());
        }
        self.state = BetPlacementState::Prepared;
        self.add_event(BetPlacementEvent::Prepared {
            timestamp: Utc::now(),
        });
        Ok(())
    }

    pub fn mark_validated(&mut self) -> Result<(), String> {
        if self.state != BetPlacementState::Prepared {
            return Err("Invalid state for validated".into());
        }
        self.state = BetPlacementState::Validated;
        self.add_event(BetPlacementEvent::Validated {
            timestamp: Utc::now(),
        });
        Ok(())
    }
    ...
    pub fn validate_exposure(&mut self) -> Result<(), String> {
        if self.state != BetPlacementState::Validated && self.state != BetPlacementState::Created {
    ...
    pub fn cancel(&mut self, reason: String) -> Result<(), String> {
        if matches!(
            self.state,
            BetPlacementState::Confirmed | BetPlacementState::Placed | BetPlacementState::Executing
        ) {
...
mod tests {
    ...
    fn test_create_state_machine() {
        ...
        assert_eq!(machine.state, BetPlacementState::Pending);
    ...
    fn test_validate_exposure() {
        ...
        assert!(machine.mark_validated().is_ok());
    ...
    fn test_validate_exposure_invalid_state() {
        let command_id = Uuid::new_v4();
        let mut machine = BetPlacementStateMachine::new(command_id);
        machine.state = BetPlacementState::Pending;
