...
impl ExposureTracker {
    ...
    pub fn get_total_exposure(&self) -> f64 {
        ...
        self.by_bookmaker
            .values()
            .sum::<f64>()
            .max(self.by_event.values().sum::<f64>())
    }
}
...
mod tests {
    ...
    fn test_exposure_validator_bookmaker_limit() {
        ...
        let result =
            validator.can_place_bet("Pari", "event-1", "League1", "Football", 8000.0, bankroll);
        assert!(result.is_ok());
        ...
        // Должна не пройти
        let result =
            validator.can_place_bet("Pari", "event-2", "League1", "Football", 3000.0, bankroll);
    ...
    fn test_exposure_validator_event_limit() {
        ...
        let result =
            validator.can_place_bet("Pari", "event-1", "League1", "Football", 4000.0, bankroll);
        assert!(result.is_ok());
        ...
        let result =
            validator.can_place_bet("Fonbet", "event-1", "League1", "Football", 2000.0, bankroll);
        assert!(result.is_err());
    }
    ...
    fn test_exposure_validator_league_limit() {
        ...
        let result =
            validator.can_place_bet("Pari", "event-1", "EPL", "Football", 12000.0, bankroll);
        assert!(result.is_ok());
    }
    ...
    fn test_exposure_validator_reset() {
        ...
        assert_eq!(
            validator.get_tracker().get_bookmaker_exposure("Pari"),
            5000.0
        );
