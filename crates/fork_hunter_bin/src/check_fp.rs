...
async fn main() -> anyhow::Result<()> {
    ...
    let result_a = tokio::time::timeout(std::time::Duration::from_secs(90), parser_a.fetch_all())
        .await
        .unwrap()
        .unwrap();

    println!("Fetching {}...", parser_b.slug());
    let result_b = tokio::time::timeout(std::time::Duration::from_secs(90), parser_b.fetch_all())
        .await
        .unwrap()
        .unwrap();
        ...
        if fp_to_events_b.contains_key(fp) {
            ...
            if matched <= 3 {
                ...
                if let Some(ea) = events_a.first() {
                    println!(
                        "   {}: {} vs {}",
                        ea.bookmaker_slug, ea.home_team, ea.away_team
                    );
                }
                if let Some(eb) = events_b.first() {
                    println!(
                        "   {}: {} vs {}",
                        eb.bookmaker_slug, eb.home_team, eb.away_team
                    );
                }
                println!();
            }
    ...
    println!(
        "📈 Matching: {}/{} ({:.1}%) events from {} match with {}",
        matched,
        total_a,
        if total_a > 0 {
            matched as f64 / total_a as f64 * 100.0
        } else {
            0.0
        },
        parser_a.slug(),
