...
async fn main() -> anyhow::Result<()> {
    ...
    let result_a = match tokio::time::timeout(
        ...
        Ok(Ok(r)) => Some(r),
        Ok(Err(e)) => { println!("❌ Error: {}", e); None }
        Err(_) => { println!("⏱️  Timeout"); None }
    };
    ...
    let (events_a, odds_a) = result_a.map(|r| (r.events, r.odds)).unwrap_or((vec![], vec![]));
    let (events_b, odds_b) = result_b.map(|r| (r.events, r.odds)).unwrap_or((vec![], vec![]));
    
    println!("\n📊 Results:");
    println!("  {}: {} events, {} odds", parser_a.slug(), events_a.len(), odds_a.len());
    println!("  {}: {} events, {} odds", parser_b.slug(), events_b.len(), odds_b.len());
    ...
    // Normalizer
    let norm = Normalizer::new();
    let norm_events_a: Vec<_> = events_a.iter().map(|e| norm.normalize_event(e.clone())).collect();
    let norm_events_b: Vec<_> = events_b.iter().map(|e| norm.normalize_event(e.clone())).collect();
    
    // Fingerprints
    println!("\n🔍 Fingerprint analysis (first 10 from each):");
    println!("\n{} fingerprints:", parser_a.slug());
    let fps_a: Vec<_> = norm_events_a.iter().take(10).map(|e| {
    ...
    let fps_b: Vec<_> = norm_events_b.iter().take(10).map(|e| {
        let fp = event_fingerprint(e);
        println!("  {} vs {} → {}", e.home_team, e.away_team, fp);
        fp
    }).collect();
    ...
    let multi_bk = all_matches.values().filter(|evs| {
        let bks: std::collections::HashSet<_> = evs.iter().map(|e| e.bookmaker_slug.as_str()).collect();
        bks.len() >= 2
    }).count();
    ...
    for (fp, events) in &all_matches {
        if shown >= 5 { break; }
        let bks: std::collections::HashSet<_> = events.iter().map(|e| e.bookmaker_slug.as_str()).collect();
        if bks.len() >= 2 {
            println!("\n  #{} FP: {}", shown + 1, fp);
            println!("    BKs: {:?}", bks);
            ...
            // Show market types
            let markets: std::collections::HashSet<_> = events.iter()
                .flat_map(|e| {
                    odds_a.iter().chain(odds_b.iter())
                        .filter(|o| o.event_id == e.id)
                        .map(|o| o.market.clone())
                }).collect();
            println!("    Markets: {:?}", markets.iter().take(5).collect::<Vec<_>>());
    ...
    let all_events: Vec<_> = norm_events_a.into_iter().chain(norm_events_b.into_iter()).collect();
