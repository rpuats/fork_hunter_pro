...
async fn main() -> anyhow::Result<()> {
    ...
    let result_24bet = tokio::time::timeout(
        std::time::Duration::from_secs(60),
        parser_24bet.fetch_all()
    ).await.unwrap().unwrap();
    ...
    println!("\n📊 24bet: {} events, {} odds", result_24bet.events.len(), result_24bet.odds.len());
    println!("📊 Bettery: {} events, {} odds\n", result_bettery.events.len(), result_bettery.odds.len());
    ...
    // Bettery
    let battery_events: Vec<_> = result_bettery.events.iter()
        .map(|ev| norm.normalize_event(ev.clone())).collect();
    ...
    for (fp, odds) in &fp_to_odds {
        ...
        if has_1 && has_x && has_2 {
            ...
            if count_full_1x2 <= 3 {
                ...
                println!("   1: {} odds (best: {:.2})", ones.len(), ones.iter().map(|o| o.odds).fold(0.0f64, f64::max));
                println!("   X: {} odds (best: {:.2})", xs.len(), xs.iter().map(|o| o.odds).fold(0.0f64, f64::max));
                println!("   2: {} odds (best: {:.2})", twos.len(), twos.iter().map(|o| o.odds).fold(0.0f64, f64::max));
                ...
                if let Some(profit) = shared::odds::calculate_surebet_profit(&[best_1, best_x, best_2]) {
