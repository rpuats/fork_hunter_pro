...
async fn main() -> anyhow::Result<()> {
    ...
    println!("\n📊 TOTAL: {} events, {} odds\n", all_events.len(), all_odds.len());
    
    // Normalize events
    let norm = Normalizer::new();
    let norm_events: Vec<_> = all_events.iter()
    ...
    let multi_bk: Vec<_> = odds_by_match.iter()
        .filter(|(_, odds)| {
    ...
    println!("🔗 Matched: {} total, {} with 2+ BKs\n", matches.len(), multi_bk.len());
    ...
    for (fp, odds) in &odds_by_match {
        ...
        for (market, market_odds) in &by_market {
            ...
            if lower.starts_with("1x2") || lower.starts_with("исход") || lower.starts_with("match") {
                let ones: Vec<_> = market_odds.iter().filter(|o| o.selection == "1" || o.selection.to_lowercase() == "п1").cloned().collect();
                let xs: Vec<_> = market_odds.iter().filter(|o| o.selection == "X" || o.selection.to_lowercase() == "х").cloned().collect();
                let twos: Vec<_> = market_odds.iter().filter(|o| o.selection == "2" || o.selection.to_lowercase() == "п2").cloned().collect();
                
                if ones.is_empty() || xs.is_empty() || twos.is_empty() { continue; }
                
                for &o1 in &ones {
                    for &ox in &xs {
                        for &o2 in &twos {
                            let bks: std::collections::HashSet<_> = [o1.bookmaker_slug.as_str(), ox.bookmaker_slug.as_str(), o2.bookmaker_slug.as_str()].iter().cloned().collect();
                            if bks.len() < 2 { continue; }
                            
                            if let Some(profit) = shared::odds::calculate_surebet_profit(&[o1.odds, ox.odds, o2.odds]) {
                                if profit < 0.1 { continue; }
                                
                                total_surebets += 1;
                                if total_surebets <= 5 {
                                    println!("💰 SUREBET #{}: {:.2}% profit", total_surebets, profit);
                                    println!("   Match: {}", fp);
                                    println!("   1: {} @{:.2} ({})", o1.bookmaker_slug, o1.odds, o1.selection);
                                    println!("   X: {} @{:.2} ({})", ox.bookmaker_slug, ox.odds, ox.selection);
                                    println!("   2: {} @{:.2} ({})", o2.bookmaker_slug, o2.odds, o2.selection);
                                    println!();
                                }
                            }
                        }
                ...
                for &o_over in &overs {
                    for &o_under in &unders {
                        ...
                        if let Some(profit) = shared::odds::calculate_surebet_profit(&[o_over.odds, o_under.odds]) {
                            ...
                            total_surebets += 1;
                            if total_surebets <= 5 {
                                println!("💰 SUREBET #{} (Total): {:.2}% profit", total_surebets, profit);
                                println!("   Match: {}", fp);
                                println!("   Over: {} @{:.2} line={:?}", o_over.bookmaker_slug, o_over.odds, o_over.line);
                                println!("   Under: {} @{:.2} line={:?}", o_under.bookmaker_slug, o_under.odds, o_under.line);
                                println!();
                            }
                        }
                    }
    ...
    if total_surebets == 0 {
        ...
        for (market, (count, multi)) in stats_vec.iter().take(15) {
            let pct = if *count > 0 { (*multi as f64 / *count as f64) * 100.0 } else { 0.0 };
            println!("  {:30} {:5} matches ({:5} multi-BK, {:.0}%)", market, count, multi, pct);
        }
    }
