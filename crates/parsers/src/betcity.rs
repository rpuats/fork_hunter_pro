...
impl BetcityParser {
    ...
    fn call_python_parser(&self) -> Vec<PyEvent> {
        let script_path = "scanner/parsers/parse_betcity_json.py";
        debug!(script = script_path, "Calling Python Betcity parser");
        ...
        match output {
            Ok(output) if output.status.success() => {
                ...
                if let Ok(events) = serde_json::from_str::<Vec<PyEvent>>(&stdout) {
                    debug!(count = events.len(), "Python Betcity parser returned events");
                    events
                } else {
    ...
    fn convert_events(&self, py_events: &[PyEvent]) -> (Vec<Event>, Vec<Odd>) {
        ...
        for py_event in py_events {
            if let (Some(home), Some(away)) = (&py_event.home_team, &py_event.away_team) {
                ...
                let event_id = format!("betcity-{}-{}", home.replace(' ', "_"), away.replace(' ', "_"));
                ...
                if let Some(odds) = py_event.home_odds {
                    if odds > 1.0 {
                        all_odds.push(Odd { id: format!("{}-1", event_id), event_id: event_id.clone(), bookmaker_slug: "betcity".to_string(), market: "1X2".into(), selection: "1".into(), odds, odds_type: OddsType::Home, line: None, timestamp: now });
                    }
                }
                if let Some(odds) = py_event.draw_odds {
                    if odds > 1.0 {
                        all_odds.push(Odd { id: format!("{}-x", event_id), event_id: event_id.clone(), bookmaker_slug: "betcity".to_string(), market: "1X2".into(), selection: "X".into(), odds, odds_type: OddsType::Draw, line: None, timestamp: now });
                    }
                }
                if let Some(odds) = py_event.away_odds {
                    if odds > 1.0 {
                        all_odds.push(Odd { id: format!("{}-2", event_id), event_id: event_id.clone(), bookmaker_slug: "betcity".to_string(), market: "1X2".into(), selection: "2".into(), odds, odds_type: OddsType::Away, line: None, timestamp: now });
                    }
                }
            }
        }
...
impl BookmakerParser for BetcityParser {
    fn name(&self) -> &str { "Betcity" }
    fn slug(&self) -> &str { "betcity" }
    fn is_enabled(&self) -> bool { true }

    async fn fetch_events(&self) -> Result<Vec<Event>, Box<dyn std::error::Error + Send + Sync>> {
        info!("Betcity: calling Python parser...");
        ...
        info!(count = events.len(), "Betcity events parsed via Python");
        Ok(events)
    }
    ...
    async fn fetch_all(&self) -> Result<ParserResult, Box<dyn std::error::Error + Send + Sync>> {
        ...
        info!(events = events.len(), odds = odds.len(), time_ms = elapsed, "Betcity fetch complete via Python");
        Ok(ParserResult::new("betcity", events, odds, elapsed))
    }

    fn base_url(&self) -> &str { "https://betcity.ru" }
