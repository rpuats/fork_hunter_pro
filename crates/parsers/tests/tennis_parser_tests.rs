...
fn factory_registers_tennis_parser() {
    ...
    let tennis_parser = factory
        .get("tennis")
        .expect("tennis parser should be registered");
...
fn factory_parser_coverage_includes_tennis() {
    ...
    assert!(
        tennis_coverage.is_some(),
        "Tennis should be in parser coverage"
    );
...
fn factory_bookmaker_metadata_includes_tennis() {
    ...
    assert!(
        tennis_metadata.is_some(),
        "Tennis should be in bookmaker metadata"
    );
...
fn factory_parser_coverage_notes_are_descriptive() {
    ...
    let has_description =
        notes.contains("3000") || notes.contains("ATP") || notes.contains("Grand Slam");
    assert!(
        has_description,
        "Tennis coverage should have descriptive notes about tournaments"
    );
}
...
fn tennis_parser_is_api_type() {
    ...
    assert_eq!(
        tennis_cov.parser_type, "api",
        "Tennis should be registered as API type parser"
    );
}
